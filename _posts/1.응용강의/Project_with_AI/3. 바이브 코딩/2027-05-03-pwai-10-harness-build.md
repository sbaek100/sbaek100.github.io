...
`claude`를 실행하고(`cwd`가 `.../pcapeek`인지 확인), 다음 지시문 전체를 붙여 넣는다. 지시문 첫머리의 「내 프로젝트」 블록(언어·표준 라이브러리·실행 방식)은 새로 정하는 것이 아니라 **기술 스택 결정표에서 그대로 옮겨 적은 것**이다 — 결정은 기획 단계에서 이미 끝났고, 여기서는 전달만 한다.
...
여기가 가장 중요한 단계이다. **하네스가 아무리 좋아도 문서가 얕으면 결과도 얕다.** 기획 챕터에서 완성한 세 산출물이 이 지시문의 재료가 되며, 새로 쓰는 것이 아니라 다음 대응대로 **옮겨 적는 것**이다.

| 기획 산출물 | 들어가는 곳 (지시문 항목 → docs/ 문서) |
|---|---|
| PRD의 개요·대상 사용자 | 프로젝트 → PRD.md |
| PRD의 In Scope (Must) | 핵심 기능 → PRD.md |
| PRD의 Out of Scope (이유 포함) | 안 만들 것 → PRD.md |
| PRD의 사용자 시나리오 (기본 흐름·잘못될 수 있는 것) | 완료 기준 → PRD.md |
| 기술 스택 결정표 (선택·버린 후보와 이유) | 결정 → ADR.md |
| 화면 설계 (원칙·와이어프레임·하지 말 것) | 출력 → OUTPUT_GUIDE.md |

다시 `claude`를 실행하고 붙여 넣는다. 아래는 pcapeek의 산출물로 채운 완성본이며, 자신의 프로젝트라면 각 항목에 자기 산출물을 같은 대응으로 넣으면 된다.
