...
- **지난 강의 도달 상태**: DMZ에 취약 웹서버(VM-WEB, 192.168.10.10) 구축, Nginx 접근 로그 생성 확인
- **오늘의 목표**: LAN에 **관제 서버 VM3(192.168.1.100)** 를 만들고 **Wazuh(SIEM)** 직접 설치 → VM-WEB에 **센서(에이전트)**를 붙여 웹 로그를 관제실로 전송
- **오늘 켜는 VM**: VM1 + VM2 + VM-WEB + **VM3(신규)**
- **오늘 완성되는 연결**: `VM-WEB(센서) → [방화벽: DMZ→192.168.1.100 허용] → VM3 Wazuh 관제실`

```mermaid
graph LR
    WEB["VM-WEB 센서<br/>192.168.10.10"] -->|"1514/1515<br/>(DMZ→LAN 예외 허용)"| FW["방화벽"]
    FW --> VM3["VM3 Wazuh 관제서버<br/>192.168.1.100"]
...
### [단계 2] Ubuntu Server 설치 & 고정 IP 192.168.1.100
...
- `sudo tee .../99-soc.yaml <<EOF ... EOF` : 새 설정을 저장합니다. **이번엔 LAN망**이라 고정 IP `192.168.1.100/24`, 게이트웨이·DNS는 방화벽 `192.168.1.1`을 씁니다(30강 웹서버는 DMZ의 `192.168.10.x`였던 것과 대비).
- `sudo chmod 600` / `sudo netplan apply` : 권한을 좁히고 설정을 즉시 반영. 이 순간 VM3의 IP가 `192.168.1.100`으로 고정됩니다.
- `ip addr | grep 192.168.1.100` : 고정 IP가 적용됐는지 한 줄로 확인.
...
- 설치가 끝나면 화면 맨 아래에 **관리자 계정 정보**가 출력됩니다. **반드시 메모하세요.**
  ```text
  INFO: --- Summary ---
  INFO: You can access the web interface https://192.168.1.100
      User: admin
...
1. **VM2** Firefox 주소창에 **`https://192.168.1.100`** 입력.
...
2. 옵션을 고릅니다.
   ...
   - **Server address**: **`192.168.1.100`** ← 관제 서버 IP
   - **Agent name**: `web-dmz`
...
# 2) 관제서버(192.168.1.100)를 가리키며 에이전트 설치
sudo WAZUH_MANAGER='192.168.1.100' WAZUH_AGENT_NAME='web-dmz' apt install -y wazuh-agent
```
...
- `sudo WAZUH_MANAGER='192.168.1.100' WAZUH_AGENT_NAME='web-dmz' apt install -y wazuh-agent` : 센서(`wazuh-agent`)를 설치합니다. 앞에 붙인 두 값이 핵심입니다.
  - `WAZUH_MANAGER='192.168.1.100'` : 이 센서가 로그를 보낼 **관제서버(VM3)의 주소**. 잘못 넣으면 센서가 관제실에 연결되지 않습니다.
...
2. `web-dmz` 에이전트가 **Active(초록색)** 상태로 1대 보이면 센서 연결 성공입니다!
   ...
   {: .prompt-tip }
3. (확인) VM2 브라우저에서 **DVWA에 로그인된 상태(보안레벨 Low)**로 `http://192.168.10.10/vulnerabilities/sqli/?id=1' UNION SELECT user,password FROM users-- -&Submit=Submit` 를 한 번 더 보낸 뒤, 잠시 후 대시보드에 이벤트가 쌓이는지 봅니다. (자세한 분석은 32강)

---

## 5. 자주 나는 오류

- **에이전트가 `Active`가 안 되고 `Never connected`**: ① VM-WEB에서 `ping 192.168.1.100` 되는지 확인(29강 **DMZ→192.168.1.100 허용 규칙** 필수), ② `WAZUH_MANAGER` IP를 192.168.1.100으로 정확히 넣었는지 확인.
...
- 센서→관제실 통신은 29강에서 만든 **DMZ→192.168.1.100 예외 허용** 규칙 덕분에 망 분리를 유지한 채 작동합니다.
