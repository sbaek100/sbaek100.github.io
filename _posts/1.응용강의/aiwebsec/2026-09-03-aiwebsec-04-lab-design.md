---
title: "[AI 보안 자동화 Lab] 01-00. 격리 랩 설계 — 왜 격리하고 무엇을 관측하는가"
...
## 중요 — "설치할 때만 잠깐 인터넷, 실습할 때는 격리"

여기서 초보자가 가장 많이 막히는 지점을 짚고 넘어갑니다. 방금 우리는 랩을 **외부 인터넷과 끊었습니다.** 그런데 바로 다음 05강부터는 `apt install`, `git clone`, Ollama 설치(`curl … ollama.com`), `ollama pull`, `pip install`처럼 **인터넷에서 무언가를 내려받는 명령**이 계속 나옵니다. 격리된 상태 그대로면 05강 첫 명령 `sudo apt update`부터 실패합니다.

해결책은 간단합니다. **설치는 인터넷을 켜고, 실습(공격 자동화)은 인터넷을 끄고** 하면 됩니다. VM에 어댑터를 하나 더 두고 필요할 때만 켜는 방식입니다.

**권장 구성 — 어댑터 2개**

| 어댑터 | 연결 방식 | 용도 | 평소 상태 |
| --- | --- | --- | --- |
| 어댑터 1 | **내부 네트워크**(`aiweblab`) | VM ↔ VM 실습 통신(고정 IP) | 항상 켬 |
| 어댑터 2 | **NAT** | 도구·모델·패키지 **다운로드 전용** | 설치할 때만 켜고, 끝나면 끔 |

VirtualBox에서 각 VM `설정 → 네트워크 → 어댑터 2` 탭을 열어 `네트워크 어댑터 사용하기`를 체크하고 `다음에 연결됨`을 **NAT**로 지정하면 됩니다.

**작업 순서**

1. **설치 단계**(05·06·07강 등): 어댑터 2(NAT)를 **켠 상태**로 `apt install`·Ollama 설치·`ollama pull`·`pip install`을 모두 마칩니다.
2. **실습 단계**(08강 이후 공격 자동화): 어댑터 2(NAT)를 **끕니다.** 이때부터 모든 트래픽은 내부 네트워크 안에만 머뭅니다. 로컬 LLM(Ollama)은 이미 내려받았으므로 인터넷 없이도 동작합니다.

> **왜 굳이 껐다 켜나요?** 자동화 공격 스크립트가 실수로 외부를 향하더라도 트래픽이 랩 밖으로 나가지 못하게 하려는 것입니다. 특히 08강부터는 AI가 스스로 도구를 실행하므로, 실습 중에는 NAT를 꺼 두는 습관이 안전합니다. **설치가 끝난 로컬 모델·도구는 인터넷 없이 동작하므로 실습에 지장이 없습니다.**
{: .prompt-warning }

> **더 엄격하게 하고 싶다면**: 어댑터 2를 아예 두지 않고, 설치가 필요할 때만 어댑터 1을 잠시 **NAT**로 바꿨다가 설치 후 다시 **내부 네트워크**로 되돌리는 방법도 있습니다. 다만 매번 IP를 다시 잡아야 해서 번거로우므로, 입문 단계에서는 위의 "어댑터 2개" 방식을 권장합니다.
{: .prompt-tip }

> **6부(SIEM, 28~35강)는 네트워크 구성이 다릅니다.** 6부는 방화벽(OPNsense)을 중심으로 WAN(NAT)·LAN·DMZ를 나누는 별도 구성을 씁니다. 28~29강에서 처음부터 다시 안내하므로, 1~5부의 `aiweblab` 구성과 혼동하지 마세요.
{: .prompt-info }
