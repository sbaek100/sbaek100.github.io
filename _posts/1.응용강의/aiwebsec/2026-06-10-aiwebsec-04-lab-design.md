...
graph LR
    subgraph LAB["내부 네트워크 (외부 인터넷 차단)"]
        K["Kali Linux<br/>192.168.0.10<br/>공격자 / AI 에이전트<br/>(LLM + 보안도구 + 우리 코드)"]
        U["Ubuntu 24.04<br/>192.168.0.30<br/>Victim<br/>(DVWA + 로그)"]
    end
    K -- "공격 요청" --> U
    U -- "로그 = 흔적" --> K
```
...
| 고정 IP | 192.168.0.10 | 192.168.0.30 |
...
내부 네트워크에는 외부 DHCP 서버가 없으므로, 각 VM이 IP를 스스로 정해야 합니다. 우리 계획은 Kali에 `192.168.0.10`, Ubuntu에 `192.168.0.30`을 고정으로 줍니다. 실제 OS 안에서 고정 IP를 설정하는 명령은 다음과 같은 형태입니다(상세 적용은 05강에서 진행합니다).

```bash
# Ubuntu(Victim) 예시 — 인터페이스 이름은 환경에 따라 다를 수 있습니다
sudo ip addr add 192.168.0.30/24 dev enp0s3
