...
> 만약 `target`에 `192.168.0.30; rm -rf /` 같은 값이 들어오면, 세미콜론 뒤 명령까지 실행돼 버립니다. 이것이 바로 우리가 뒤에서 점검할 **명령어 삽입(Command Injection)** 취약점입니다.
...
- 그러면 LLM은 글로 답하는 대신, **"run_nmap을 target=192.168.0.30으로 실행하라"는 지시서**를 돌려줄 수 있습니다. 이게 의사의 진찰 결과입니다.
