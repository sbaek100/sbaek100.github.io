...
graph TB
    INET["🌐 인터넷"]
    FW["VM1 · OPNsense 방화벽<br/>WAN(NAT) 10.0.2.15<br/>LAN 192.168.1.1 · DMZ 192.168.10.1"]
    VM2["VM2 · Ubuntu Desktop<br/>학생 워크스테이션 / 공격자 / Jupyter<br/>192.168.1.20"]
    VM3["VM3 · SOC 통합서버<br/>Wazuh · Ollama · 자동대응(Integrator)<br/>192.168.1.100"]
    WEB["VM-WEB · 취약 웹서버<br/>DVWA + Nginx<br/>192.168.10.10"]

    INET --> FW
    FW -->|"intnet_lan<br/>192.168.1.0/24"| VM2
    FW -->|"intnet_lan<br/>192.168.1.0/24"| VM3
    FW -->|"intnet_dmz<br/>192.168.10.0/24"| WEB
```
...
| LAN (내부 안전망) | 내부 네트워크 `intnet_lan` | 192.168.1.0/24 | 192.168.1.1 (OPNsense) |
| DMZ (노출 구역) | 내부 네트워크 `intnet_dmz` | 192.168.10.0/24 | 192.168.10.1 (OPNsense) |

| 장비 | 고정 IP |
|---|---|
| OPNsense LAN | 192.168.1.1 |
| OPNsense DMZ | 192.168.10.1 |
| VM2 워크스테이션 | 192.168.1.20 |
| VM3 SOC 서버 | 192.168.1.100 |
| VM-WEB 웹서버 | 192.168.10.10 |
...
| OPNsense 웹관리 | https://192.168.1.1 | `root` | `opnsense123` (설치 중 설정) |
| Wazuh 대시보드 | https://192.168.1.100 | `admin` | (설치 시 자동 생성, 31강에서 메모) |
| OPNsense API (자동차단용) | 192.168.1.1 (key/secret) | — | (34강에서 발급) |
...
29강에서는 **OPNsense 방화벽(VM1)**을 직접 설치하고, 오늘 만든 VM2를 **안전한 내부망(LAN, 192.168.1.0/24)**으로 옮겨 연결합니다.
