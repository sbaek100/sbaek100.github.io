---
title: "[AI 보안 자동화 Lab] 09-01. SIEM 실전 관제 실습 ② — 가상 방화벽으로 안전구역 만들기"
date: 2026-12-12 09:00:00 +0900
categories:
  - 1.응용강의
  - AI자동화
tags:
...
- **DMZ 구역(`intnet_dmz`, 192.168.59.0/24)**: 홈페이지처럼 **외부에 노출되어 공격받기 쉬운** 서버(=VM-WEB)를 두는 곳
- **LAN 구역(`intnet_lan`, 192.168.58.0/24)**: 관제 서버·관리자 PC 등 **소중한 자산**을 두는 안전한 곳
...
graph LR
    ...
    FW -->|LAN 192.168.58.0/24| L["VM2 · (이후 VM3)"]
    FW -->|DMZ 192.168.59.0/24| D["(이후 VM-WEB)"]
```
...
```text
 ...
 LAN (em1)  -> v4: 192.168.58.1/24         (내부 안전망, 기본값)
```
...
> OPNsense는 **LAN(192.168.58.1)에서 들어오는 관리 접속을 기본 허용(anti-lockout)**합니다. 그래서 곧 VM2(LAN)의 브라우저로 방화벽 설정 화면에 접속할 수 있습니다.
{: .prompt-tip }
...
3. 이어서 아래 세 명령으로 단계별로 확인합니다.
   ...
   **명령어 상세 설명**
   - `ip addr show` : 이 PC에 붙은 모든 네트워크 카드와 **현재 할당된 IP 주소**를 보여줍니다. 출력 중 유선 카드(`enp0s3` 등) 항목에 `inet 192.168.58.20/24` 가 보이면, 앞 단계에서 준 고정 IP가 정상 적용된 것입니다. (`addr`=address(주소), `show`=보여줘)
   - `ping -c 3 192.168.58.1` : 방화벽(게이트웨이)에게 **"살아 있니?"** 하고 신호를 3번 보냅니다. (`ping`=네트워크 응답 확인 도구, `-c 3`=count(횟수) 3번만 보내고 멈춤) `3 received, 0% packet loss` 가 나오면 VM2 ↔ 방화벽 통신이 정상입니다. 즉 **같은 내부망(`intnet_lan`)에 올바로 묶였다**는 뜻입니다.
...
1. VM2의 Firefox 주소창에 **`https://192.168.58.1`** 입력.
...
4. 상단 메뉴 **[Interfaces] → [Assignments]** 로 가면 `OPT1`이 보입니다. **[OPT1]** 클릭 후:
   ...
   - **IPv4 address**: **`192.168.59.1`** / `24`
   - 하단 **[Save] → [Apply changes]**

### [단계 8] DMZ DHCP는 건너뜀 (VM-WEB은 고정 IP 사용)

이번 실습에서는 DMZ 서버에 **고정 IP(192.168.59.10)**를 직접 줄 것이므로 DHCP는 필수가 아닙니다. (참고: LAN은 기본 DHCP가 켜져 있지만, 우리는 VM2/VM3에 고정 IP를 직접 부여합니다.)
...
- **DMZ 탭**: 기본은 모두 차단입니다. 아래 두 규칙을 **[+ Add]** 로 추가합니다.
  ...
  2. **DMZ → 관제서버(Wazuh)만 허용** (웹서버 센서가 로그를 보내도록)
     - Action: `Pass`, Source: `DMZ net`, Destination: **`192.168.58.100`** (Single host), Destination port: `any`
...
- **`https://192.168.58.1` 접속 안 됨**: VM2 어댑터가 `intnet_lan`인지, VM2 IP가 192.168.58.20인지 확인. OPNsense·VM2 둘 다 `intnet_lan`으로 이름이 **정확히 같아야** 통신됩니다.
...
30강에서는 **DMZ에 취약 웹서버 VM-WEB(192.168.59.10)**을 직접 만들어 **DVWA(Damn Vulnerable Web Application)**를 띄우고, VM2에서 첫 해킹(SQL Injection)을 시도해 봅니다.
