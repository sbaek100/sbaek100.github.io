---
title: "[AI 보안 자동화 Lab] 00-00. 시리즈 개요 — 취약점 점검에서 자동화된 보안관제까지"
...
강의 코드는 **`부-순번`** 형식입니다. 예를 들어 `02-01`은 **2부의 두 번째 강의**를 뜻합니다.

| 부 | 강의 코드 | 핵심 |
|---|---|---|
| **0부. 서론·배경** | `00-00` 시리즈 개요 (이 글) | 전체 그림, 도착점 = 자동화된 보안관제 |
| | `00-01` 위협 환경과 침해사고 | 공격 표면, 침해사고의 의미, 사례 |
| | `00-02` 사이버 킬체인 | 공격자는 어떻게 들어오는가 |
| | `00-03` 취약점의 공통 언어 | CWE·CVE·CVSS·KEV |
| **1부. Victim 설계** | `01-00` 격리 랩 설계 원칙 | 왜 격리하고 무엇을 관측하는가 |
| | `01-01` Victim 구축 | DVWA + 로그/텔레메트리 발생원 |
| **2부. 에이전트 기초** | `02-00`~`02-03` | Ollama·LLM·도구 호출·정찰 자동화 |
| **3부. 점검 자동화** | `03-00`~`03-02` | 웹 스캔·SQLi·XSS/명령삽입 + CWE 매핑 |
| **4부. 자율화·리포트** | `04-00`~`04-02` | 가드레일·리포트(CVE/CVSS·MCP)·오케스트레이션 |
| **5부. 탐지(Blue Team)** | `05-00`~`05-01` | 공격 흔적·로그·도구 서명, 이상징후 탐지(Triage·IoC) |
| **6부. 침해사고 대응** | `06-00`~`06-01` | KISA 7단계, 사고 조사·증거 보존·신고 |
| **7부. 자동화된 보안관제** | `07-00`~`07-02` | 보안관제(SOC) 개념, AI 자동 관제 구축, Red·Blue·Purple |
| **8부. 웹 대시보드** | `08-00`~`08-04` | Streamlit으로 점검·탐지·리포트를 화면에 |
| **9부. SIEM 실전 관제 실습** | `09-00`~`09-07` | 방화벽·DMZ 구성, Wazuh SIEM, 실시간 탐지·SOAR 자동 차단 |
| **부록** | `부록 A` | AI 자체의 보안 — 프롬프트 인젝션 |

> 0~3부가 **공격(점검) 자동화**라면, 5~7부는 그것을 **탐지·대응·관제의 맥락**에 놓습니다. 이어 8부에서 결과를 **웹 대시보드**로 제품화하고, 9부에서 방화벽·DMZ·SIEM(Wazuh)을 갖춘 **실전 관제 환경**을 직접 구축합니다. 공격을 만들 줄 아는 사람이, 그 공격이 로그에 어떻게 남고 어떻게 탐지·대응되는지까지 알아야 진짜 보안 전문가입니다.
{: .prompt-info }
