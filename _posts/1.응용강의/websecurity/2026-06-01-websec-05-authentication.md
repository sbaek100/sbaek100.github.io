...
| 연도 | 사건 | 내용 |
|---|---|---|
| 2009 | **RockYou**[^c05_rockyou] | 3,200만 평문 비밀번호 유출 → 사전 공격용 `rockyou.txt`의 기원 |
| 2012 | **LinkedIn**[^c05_linkedin] | 솔트 없는 SHA-1 해시 대량 유출(이후 1억 1,700만 건으로 확인) → 재사용 계정을 노린 **크리덴셜 스터핑**의 연료가 됨 |
| 2014 | **iCloud "Celebgate"**[^c05_icloud] | Find My iPhone API에 **로그인 시도 횟수 제한이 없어** 무차별 대입 도구(iBrute)가 통함 + 피싱 → 유명인 사진 유출. 사건 후 Apple이 **Rate Limit** 도입 |
| 2016 | **Mirai 봇넷**[^c05_mirai] | IoT 기기의 **기본 계정(admin/admin 등)** 을 사전 대입으로 장악 → 사상 최대급 DDoS(Dyn 마비) |
...
## 9. 약한 비밀번호 정책 점검 (KISA 보강)

앞 절들이 **추측 횟수**를 노렸다면, 약한 비밀번호 정책은 **추측 자체를 쉽게** 만드는 설계 결함입니다. KISA *상세가이드* 의 「약한 비밀번호 정책(BF)」 점검 항목을 추가로 다룹니다.

### 9.1 점검 (KISA Step 1~2)

- **Step 1**: 추측 가능한 계정/비밀번호로 로그인 가능한 계정이 있는지 (예: `admin / 1234`)
- **Step 2**: 회원가입·정보수정·비밀번호 변경 페이지에서 **복잡도 요구가 강제되는지**

| 분류 | 취약 예시 |
|---|---|
| 취약한 ID | admin, administrator, manager, guest, tomcat, root, user, operator, anonymous |
| 취약한 PW | 1234, 1111, abcd, test, password, public, 빈 비밀번호, ID와 동일 |

취약 환경 재현 — 약한 비번을 그대로 받는 회원가입(`/var/www/html/signup.php`):

```php
<?php
// [의도적 취약] 복잡도 검증 없이 저장
$pw = $_POST['pw'] ?? '';
echo strlen($pw) > 0 ? "가입 완료(검증 없음): $pw" : "비밀번호 입력";
?>
```
...
> 문자 종류 = ① 영문 대문자(26) ② 영문 소문자(26) ③ 숫자(10) ④ 특수문자(약 32). 연속 숫자·생일·전화번호·ID 유사 비밀번호는 금지하고, 유효기간(반기 1회 변경)·최근 사용 비밀번호 재사용 금지를 함께 적용합니다.
{: .prompt-tip }

### 9.3 조치 — 모든 페이지에 동일 검증

```javascript
// 복잡도 검증 (클라이언트 보조 — 서버에서도 동일 검증 필수)
function isPasswordStrong(pw) {
  ...
  return pw.length >= minLength && hasUpper && hasLower && hasNum && hasSpec;
}
```

1. 테스트/관리자 계정의 취약한 ID·비밀번호 사용 금지
2. **회원가입·정보변경·비밀번호변경** 페이지 **모두**에 정규식 복잡도 검증 구현(서버 사이드)
3. 입력 실패 횟수 제한(3~5회 초과 시 계정 잠금) — 자동화 공격 방어와 연계

---

## 10. 자동화 공격 점검 (KISA 보강)

무차별 대입(2~5장)은 **로그인**에 대한 자동화 공격입니다. KISA 「자동화 공격(AU)」 항목은 이를 **로그인 외 프로세스**(게시글 등록, SMS 발송, 본인인증 등)로 넓혀, **반복 요청 통제 여부**를 점검합니다.

### 10.1 점검 (KISA Step 1~2)

- **Step 1**: 로그인 실패가 일정 횟수를 넘어도 **CAPTCHA·잠금 없이** 반복 요청이 통하는지
- **Step 2**: 본인인증(계좌 인증·SMS 발송 등)을 **반복 요청**해도 통제되지 않는지(문자 폭탄·요금 유발)

### 10.2 DVWA 실습 — Insecure CAPTCHA
...
| 레벨 | 문제점 | 우회 |
|---|---|---|
| **Low** | CAPTCHA 통과 여부를 **클라이언트 파라미터(`step`)** 로만 판단 | 요청에서 `step=2`로 바로 전송 → CAPTCHA 생략 |
| **Medium** | `passed_captcha` 숨은 필드를 신뢰 | Burp로 `passed_captcha=true` 추가 |
| **High/Impossible** | 서버에서 reCAPTCHA 응답을 재검증 | 우회 불가(근본) |

Burp Suite로 요청을 가로채 CAPTCHA 검증 단계를 건너뛰는 파라미터를 주입하면, 비밀번호 변경이 **CAPTCHA 없이** 처리됩니다.

### 10.3 조치

1. 로그인·게시글 등록·본인인증(계좌/SMS)에 **횟수 제한** 또는 **CAPTCHA(서버 검증)** 적용
2. 짧은 시간 다량 요청을 탐지·차단하는 **IDS/IPS·WAF** 도입
3. CAPTCHA는 반드시 **서버 사이드에서 재검증**(클라이언트 플래그 신뢰 금지)

> 자동화 공격 방어의 함정: **CAPTCHA를 넣었다는 사실**이 아니라 **서버가 그 결과를 검증하는가**가 핵심입니다. DVWA Low/Medium이 그 차이를 보여 줍니다.
{: .prompt-warning }
...
- "Credential stuffing" 용어 — Sumit Agarwal(Shape Security 공동창업자). Wikipedia. <https://en.wikipedia.org/wiki/Credential_stuffing>
- OWASP Authentication Cheat Sheet — <https://cheatsheetseries.owasp.org/cheatsheets/Authentication_Cheat_Sheet.html>

[^c05_rockyou]: RockYou 2009 — 약 3,260만 평문 비밀번호 유출, `rockyou.txt`의 기원. Wikipedia. <https://en.wikipedia.org/wiki/RockYou>
[^c05_linkedin]: 2012 LinkedIn hack — 2012년 약 650만 해시 공개, 2016년 약 1억 1,700만 계정으로 확인. Wikipedia. <https://en.wikipedia.org/wiki/2012_LinkedIn_hack>
[^c05_icloud]: 2014 "Celebgate" — Find My iPhone API의 시도 횟수 제한 부재(iBrute) + 피싱. Wikipedia. <https://en.wikipedia.org/wiki/2014_celebrity_nude_photo_leak>
[^c05_mirai]: Mirai 봇넷 2016 — Telnet 기본 계정 사전 대입, Dyn DDoS. Wikipedia. <https://en.wikipedia.org/wiki/Mirai_(malware)>
