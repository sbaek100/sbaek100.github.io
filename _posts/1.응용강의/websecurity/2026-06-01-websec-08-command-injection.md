...
> SQLi(01)·XSS(02)와 함께 **A05 Injection**(구 A03:2021) 군에 속합니다. 단, 결과가 **OS 명령 실행(RCE)** 이라 영향도가 가장 큽니다.  
> 방어 원리는 동일합니다 — **사용자 입력을 명령 문자열에 합치지 말 것**(배열 인자 + 화이트리스트).
{: .prompt-info }
...
OS 명령 삽입은 **CGI 기반 웹의 초창기(1990년대)** 부터 존재한 고전적 취약점입니다. 웹 프로그램이 `ping`·`nslookup` 같은 시스템 명령을 호출하면서 사용자 입력을 그대로 붙였기 때문입니다.

이 부류에서 역사상 가장 충격적인 사건은 **2014년 9월의 셸쇼크(Shellshock, CVE-2014-6271)** 입니다. 프랑스의 보안 연구자 **Stéphane Chazelas**가 발견한 이 버그는, 리눅스·macOS의 기본 셸인 **bash가 환경변수에 담긴 함수 선언 뒤에 붙은 명령어까지 실행**해 버리는 결함이었습니다. 무엇보다 **CGI 웹 서버는 HTTP 헤더(User-Agent 등)를 환경변수로 변환**하기 때문에, 공격자는 헤더 한 줄로 원격 명령을 실행할 수 있었습니다.

### 주요 침해 사건

| 연도 | 사건 | 내용 |
|---|---|---|
| 2014 | **Shellshock** (CVE-2014-6271)[^c08_shellshock] | bash 환경변수 파싱 결함. **CVSS 10.0**. 공개 후 **수 시간 만에** 봇넷이 전 세계 CGI 서버를 스캔·감염 — HTTP 헤더로 즉시 RCE |
| 2014~ | **Shellshock 후속 변종** | CVE-2014-6271을 막은 패치를 우회하는 추가 CVE들이 연달아 공개되며 장기간 위협 지속 |
| 상시 | **IoT·공유기 RCE** | 다수의 라우터·IP카메라 관리 페이지가 `ping` 기능 등에 명령 삽입 취약점을 가져, 봇넷 감염의 단골 경로가 됨 |

> 셸쇼크가 무서웠던 이유는 **"공격 난이도가 0에 가까웠다"** 는 점입니다. `User-Agent` 헤더에 페이로드를 넣은 요청 한 번이면 끝이었고, 패치 전까지 거의 모든 리눅스 웹 서버가 대상이었습니다.
{: .prompt-warning }

### 왜 이 공격이 통하는가 — 근본 원리

핵심은 **셸(shell)이 문자열을 "단순 텍스트"가 아니라 "명령 스크립트"로 해석**한다는 데 있습니다. `system()`·`shell_exec()`·`os.system()` 같은 함수는 전달받은 문자열을 셸에게 넘기고, 셸은 그 안의 **메타문자**를 특별하게 처리합니다.

- `;` — 앞 명령이 끝나면 **다음 명령을 이어서** 실행
- `|` — 앞 명령의 출력을 **다음 명령의 입력**으로
- `&&` / `||` — 앞 명령의 **성공/실패에 따라** 다음 명령 실행
- `` ` `` / `$( )` — 안의 명령을 **먼저 실행해 그 결과로 치환**

개발자는 `ping -c 1 <사용자입력>` 한 줄을 의도했지만, 셸은 입력에 든 `;`를 보고 **"명령이 거기서 끝났다"** 고 판단하고 그 뒤를 **새 명령으로 실행**합니다. 즉 SQLi가 SQL 파서를, XSS가 브라우저를 속이듯, **명령 삽입은 셸 파서를 속입니다.**

```bash
# 개발자 의도: ping 한 번
ping -c 1 127.0.0.1
# 공격자 입력 127.0.0.1; cat /etc/passwd 가 들어가면 셸이 보는 것:
ping -c 1 127.0.0.1 ; cat /etc/passwd
#                    ↑ 셸: "여기서 명령 끝" → 뒤를 새 명령으로 실행
```

그래서 근본 방어는 **애초에 셸을 거치지 않는 것**입니다. Python `subprocess.run([...], shell=False)`나 PHP `escapeshellarg()`처럼 입력을 **하나의 인자(데이터)로 고정**하면, 셸이 그 안의 `;`·`|`를 명령 구분자가 아닌 **평범한 글자**로 취급합니다.
...
공격자가 임의의 시스템 명령을 서버에서 실행할 수 있게 되는 취약점입니다.
...
서버는 `ping -c 1 127.0.0.1; cat /etc/passwd` 를 그대로 실행합니다.

> **영향 범위**: 파일 열람, 데이터 삭제, 리버스 쉘 획득, 내부망 이동(Lateral Movement)까지 가능한 **고위험** 취약점입니다.
{: .prompt-danger }
...
`escapeshellarg()` 는 입력값 전체를 작은따옴표로 감싸고 내부의 따옴표를 이스케이프합니다.  
공격자의 `;`, `|`, `&&` 등이 명령 구분자가 아닌 일반 문자열로 처리됩니다.
...
> `subprocess.run()` 에서 `shell=False` 가 기본값이지만, 명시적으로 작성하는 것이 좋습니다.  
> `shell=True` 는 `os.system()` 과 동일하게 취약하므로 사용자 입력이 포함될 때는 절대 사용하지 않습니다.
{: .prompt-warning }
...
일반적인 원격 제어(Normal Shell)는 공격자가 서버에 연결을 시도합니다.  
그러나 서버에 방화벽이 있으면 **인바운드(외부 → 서버) 연결**은 차단됩니다.

**Reverse Shell**은 반대 방향입니다.  
서버(공격 대상)가 **공격자에게 먼저 연결**을 시도하여 쉘을 전달합니다.  
방화벽이 아웃바운드(서버 → 외부) 트래픽은 대부분 허용하기 때문에 우회가 가능합니다.
...
> Blind Command Injection은 결과가 안 보이더라도 명령은 서버에서 실행되고 있습니다.  
> 응답 시간 지연(Time-based) 또는 외부 서버 요청(Out-of-band)으로 탐지합니다.
{: .prompt-tip }
...
화면에 ping 결과가 출력되면 **Verbose** 방식임을 확인합니다.

### 2.2 명령어 삽입 테스트

아래 입력값들을 순서대로 시도하여 결과를 확인합니다.
...
각 입력 후 웹 페이지에 ping 결과에 이어서 명령 실행 결과가 출력되는 것을 확인합니다.

> Low 레벨은 사용자 입력에 아무런 필터링 없이 `shell_exec()` 또는 `system()` 에 그대로 전달합니다.
{: .prompt-info }
...
`www-data` 계정으로 Ubuntu 서버의 쉘을 획득한 상태입니다.

> **Reverse Shell은 공격자가 서버를 완전히 제어하는 가장 위험한 단계입니다.**  
> 실습 환경에서만 수행하고, 외부 시스템에는 절대 시도하지 않습니다.
{: .prompt-danger }
...
Reverse Shell 없이도 웹쉘을 파일로 서버에 저장하면 영구적인 접근이 가능합니다.
...
> 웹쉘은 사후에도 남아 지속적인 백도어 역할을 합니다.  
> 실습 후 반드시 삭제하고, 실제 환경에서 발견 시 파일 생성 시각·내용·연관 로그를 포렌식합니다.
{: .prompt-warning }
...
Medium 레벨은 `&&` 와 `;` 를 필터링하여 명령 삽입을 막으려 합니다.
...
이 필터는 `&&` 와 `;` 만 제거하므로 다른 구분자를 사용하면 우회 가능합니다.
...
> **단순 블랙리스트 필터링은 우회됩니다.** 구분자의 종류가 다양하기 때문에  
> 화이트리스트 방식(IP 형식만 허용)으로 검증해야 합니다.
{: .prompt-warning }
...
> 핵심: 블랙리스트(Medium/High)는 항상 빠뜨린 문자가 있습니다. 특히 **High의 "파이프+공백 vs 공백 없는 파이프" 버그**가 대표적 교훈입니다.  
> Impossible의 **입력 형식 화이트리스트 + OS 명령 미사용**(7장)만이 RCE를 차단합니다.
{: .prompt-tip }
...
가장 근본적인 방어는 **OS 명령 자체를 사용하지 않는 것**입니다.  
ping, nslookup 등의 기능이 필요하다면 OS 명령 대신 언어 내장 라이브러리를 사용합니다.
...
> **핵심**: 입력 검증은 블랙리스트가 아닌 **화이트리스트** 방식으로 합니다.  
> 허용할 값의 패턴을 정의하고 그 외는 모두 거부해야 안전합니다.
{: .prompt-tip }
...
Command Injection은 단순해 보이지만 서버의 완전한 제어권을 넘겨주는 **OWASP Top 10** 핵심 취약점입니다.
...
각 레벨의 소스 코드를 비교하면서 왜 Impossible 레벨만이 안전한지 이해하는 것이 목표입니다.

---

## 9. 정보보안기사 시험 포인트

명령 삽입은 **특수문자(구분자)의 동작**을 정확히 외우는 것이 핵심입니다. (상세 이론은 **11번 포스트 7.7장** 참고)

| 기호 | 동작 |
|---|---|
| `;` | 앞 명령 성공 여부와 무관하게 **순차 실행** |
| `\|` | 앞 명령의 **출력**을 뒤 명령의 **입력**으로 (파이프) |
| `\|\|` | 앞 명령이 **실패해야** 뒤 명령 실행 |
| `&&` | 앞 명령이 **성공해야** 뒤 명령 실행 |

> **★ 최다 출제 함정**  
> - `ls && pwd` → "`ls`가 **실패해야** `pwd` 실행"은 **틀림**. `&&`는 **성공해야** 뒤가 실행됩니다.  
> - `pwd || ls -al` → `pwd`가 **실패해야** `ls -al` 실행 (`||`는 실패 시).  
> - `ls -al | more` → `ls`의 **결과를 more의 입력으로** 사용 (파이프).  
> 방어는 **화이트리스트 입력 검증 + OS 명령 미사용(언어 내장 라이브러리)** 이 정답입니다.
{: .prompt-tip }
...
본문의 취약점·사건은 아래 출처에 근거합니다(각주 번호를 클릭하면 이동).

**더 읽어보기**

- OWASP Command Injection — <https://owasp.org/www-community/attacks/Command_Injection>

[^c08_shellshock]: Shellshock — CVE-2014-6271(bash 환경변수 파싱 결함). 발견 Stéphane Chazelas, 공개 2014-09-24, CVSS 10.0; CGI에서 HTTP 헤더가 환경변수가 되어 RCE. NVD. <https://nvd.nist.gov/vuln/detail/cve-2014-6271>
