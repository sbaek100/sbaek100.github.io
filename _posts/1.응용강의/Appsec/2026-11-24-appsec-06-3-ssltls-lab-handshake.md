...
> 이 글은 **실습** 입니다. 앞 글(**06-2**)에서 만든 HTTPS 서버(`https://192.168.56.30`)가 동작 중이어야 합니다.  
...
1. 내부망 인터페이스(192.168.56.10 트래픽이 흐르는 카드, 예 `eth1` — 02-3에서 확인한 그 카드)를 더블클릭해 캡처 시작.
2. 표시 필터에 입력:
...
캡처를 켠 채, 브라우저에서 `https://192.168.56.30` 을 **새로고침** 합니다. (이미 경고를 통과해 둔 상태)
