...
> 이 실습은 전부 **Ubuntu(192.168.56.30)** 한 대에서 진행합니다.
{: .prompt-info }
...
```mermaid
flowchart LR
    T["telnet으로 SMTP 명령 입력"] --> P["Postfix(메일서버)"] --> M["/var/mail/student<br/>(사서함)"]
```

SMTP 명령(`MAIL FROM`, `RCPT TO`, `DATA`)을 직접 쳐서 `student` 사용자에게 메일을 넣고, 도착한 메일의 **헤더**를 분석합니다.
...
`student` 사용자의 사서함에 메일이 도착했는지 봅니다.
...
`cat /var/mail/student` 출력의 **위쪽 헤더** 를 봅니다. 주요 항목의 뜻은 다음과 같습니다.
...
- [ ] `mail` 또는 `cat /var/mail/student` 로 도착 메일 확인
