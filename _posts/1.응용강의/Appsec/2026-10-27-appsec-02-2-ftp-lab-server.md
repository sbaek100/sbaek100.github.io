...
> 목표: **Ubuntu(192.168.56.30)** 에 FTP 서버(vsftpd)를 올리고, **Kali(192.168.56.10)** 에서 접속해 본다.
{: .prompt-info }
...
```mermaid
flowchart LR
    K["Kali 192.168.56.10<br/>(FTP 클라이언트)"] -- "FTP 접속(21번)" --> U["Ubuntu 192.168.56.30<br/>(vsftpd 서버)"]
```
...
- [ ] (Kali) `ftp 192.168.56.30` 로 `ftpuser` 로그인 성공(`230`)
