...
> 목표: **Ubuntu(192.168.56.30)** 의 SSH 로그인을 **비밀번호 + 일회용 코드(OTP)** 2단계로 바꾸고, **Kali(192.168.56.10)** 에서 접속해 본다.
{: .prompt-info }
...
```mermaid
flowchart LR
    K["Kali"] -- "ssh student@192.168.56.30" --> U["Ubuntu"]
    U --> P1["① 비밀번호 확인"]
    P1 --> P2["② OTP 6자리 확인"]
    P2 --> OK["로그인 성공"]
```
...
## 2. (Ubuntu) student 계정의 OTP 키 만들기

OTP를 적용할 **본인 계정(student)** 으로 설정 마법사를 실행합니다. (`sudo` 없이!)

```bash
# === Ubuntu에서, student 계정으로 실행 ===
...
- [ ] `libpam-google-authenticator` 설치, student 계정으로 `google-authenticator` 키 생성
- [ ] 비상 코드 5개 별도 보관
