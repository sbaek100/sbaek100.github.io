...
> 목표: **Ubuntu(192.168.0.30)** 에 DNS 서버(BIND9)를 올리고, `lab.local` 도메인을 만들어 **Kali(192.168.0.10)** 에서 조회한다.
{: .prompt-info }
...
```mermaid
flowchart LR
    K["Kali 192.168.0.10<br/>(질의)"] -- "dig www.lab.local" --> U["Ubuntu 192.168.0.30<br/>(BIND9 권한 서버)"]
    U -- "192.168.0.30 응답" --> K
```
...
| 이름 | 종류 | 값 |
|---|---|---|
| `ns.lab.local` | A | 192.168.0.30 |
| `www.lab.local` | A | 192.168.0.30 |
| `ubuntu.lab.local` | A | 192.168.0.30 |
| `kali.lab.local` | A | 192.168.0.10 |
| `mail.lab.local` | A | 192.168.0.30 |
...
ubuntu  IN      A       192.168.0.30
kali    IN      A       192.168.0.10
...
이제 **Kali** 에서 우리 DNS 서버(`192.168.0.30`)에게 직접 물어봅니다.

```bash
# === Kali에서 실행 ===
# A 레코드 조회
dig @192.168.0.30 www.lab.local

# 짧게 결과만
dig @192.168.0.30 www.lab.local +short      # → 192.168.0.30

dig @192.168.0.30 kali.lab.local +short     # → 192.168.0.10

# MX(메일 서버) 레코드
dig @192.168.0.30 lab.local MX +short       # → 10 mail.lab.local.

# nslookup으로도 가능
nslookup www.lab.local 192.168.0.30
```

- `@192.168.0.30` 은 "이 DNS 서버에게 물어봐" 라는 뜻입니다.
- `www.lab.local` 이 `192.168.0.30` 으로 응답되면 **우리 DNS 서버가 정상 동작** 하는 것입니다.
...
- [ ] (Kali) `dig @192.168.0.30 www.lab.local +short` → `192.168.0.30`
- [ ] (Kali) `dig @192.168.0.30 lab.local MX +short` → `mail.lab.local`
