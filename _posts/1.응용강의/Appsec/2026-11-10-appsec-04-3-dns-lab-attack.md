...
```bash
# === Kali에서 실행 ===
dig axfr lab.local @192.168.56.30
```
...
kali.lab.local.   IN  A    192.168.56.10
mail.lab.local.   IN  A    192.168.56.30
ns.lab.local.     IN  A    192.168.56.30
ubuntu.lab.local. IN  A    192.168.56.30
www.lab.local.    IN  A    192.168.56.30
```
...
> 실제 운영에서는 `none` 대신 **Secondary의 IP만** 적습니다. 예: `allow-transfer { 192.168.56.31; };`
{: .prompt-tip }
...
> 단, 개별 조회(`dig @192.168.56.30 www.lab.local`)는 여전히 정상 동작합니다. **"전체 복제(AXFR)만" 막은 것**이지 DNS 기능 자체를 끈 게 아닙니다.
{: .prompt-tip }
...
> `www.examplebank.com` 을 **공격자 IP 192.168.56.10** 으로 응답하도록 거짓 정보를 넣은 것입니다.
{: .prompt-warning }
...
```bash
# === Kali에서 실행 ===
dig @192.168.56.30 www.examplebank.com +short
```

결과는 **`192.168.56.10`**(공격자) 입니다.
