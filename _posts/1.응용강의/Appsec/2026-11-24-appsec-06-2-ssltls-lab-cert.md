...
> 목표: **Ubuntu(192.168.56.30)** 에 인증서를 만들어 **HTTPS 웹서버**를 올리고, **Kali(192.168.56.10)** 브라우저로 접속해 자물쇠·경고를 확인한다.
{: .prompt-info }
...
```mermaid
flowchart LR
    K["Kali 192.168.56.10<br/>(브라우저)"] -- "https://192.168.56.30 (443)" --> U["Ubuntu 192.168.56.30<br/>(Apache + 인증서)"]
```
...
```bash
# === Kali에서 실행 ===
echo | openssl s_client -connect 192.168.56.30:443 2>/dev/null | openssl x509 -noout -subject -issuer -dates
```
...
- [ ] (Kali) `https://192.168.56.30` 접속 시 **자체서명 경고 → 🔒 페이지** 확인
