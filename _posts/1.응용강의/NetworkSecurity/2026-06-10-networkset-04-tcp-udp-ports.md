...
> **이번 실습 대상**: Metasploitable2 `192.168.60.100`의 DVWA(`http://192.168.60.100/dvwa/`, 로그인 `admin/password`). 공격자·분석자는 Kali `192.168.60.10`.
{: .prompt-info }
...
> **목표** Kali에서 Metasploitable2의 웹(80)에 접속할 때 오가는 3-way 핸드셰이크를 캡처한다. **대상** Kali(192.168.60.10) → Metasploitable2(192.168.60.100) DVWA.
{: .prompt-tip }
...
```text
[SYN]       192.168.60.10  → 192.168.60.100   (Flags 0x002)
[SYN, ACK]  192.168.60.100 → 192.168.60.10    (Flags 0x012)
[ACK]       192.168.60.10  → 192.168.60.100   (Flags 0x010)
```
...
> **목표** hping3로 RST·FIN·PSH 패킷을 보내고 각 플래그와 서버 반응을 확인한다. **대상** Kali → Metasploitable2(192.168.60.100) 80번 포트.
{: .prompt-tip }

**RST 주입** — "당장 끊어" 강제 종료 신호

```bash
sudo hping3 -R -p 80 -s 45678 192.168.60.100
```
...
```bash
sudo hping3 -F -p 80 -s 45678 192.168.60.100
```
...
```bash
sudo hping3 -P -p 80 -d 100 192.168.60.100
```
...
```bash
watch -n 1 "netstat -ant | grep 192.168.60.100"   # ESTABLISHED, TIME_WAIT 등 관찰
```
...
> **목표** DNS 질의를 보내고 Wireshark로 UDP 요청-응답 한 쌍을 관찰한다. **대상** Kali → Metasploitable2(192.168.60.100) DNS(53).
{: .prompt-tip }

```bash
# 필터: udp && ip.dst_host == 192.168.60.100
dig @192.168.60.100 example.com
```

> **왜?** `@192.168.60.100`은 그 주소를 DNS 서버로 지정한다는 뜻입니다. UDP는 연결이 없으므로 질의와 응답을 **Transaction ID**로 짝짓습니다. 응답 패킷은 출발지·목적지 포트가 요청과 반대로 뒤집혀 있고, 두 패킷의 시간차가 곧 조회 시간입니다.
...
> **목표** UDP 포트 스캔의 응답 방식과, 출발지를 위조한 스푸핑 패킷을 관찰한다. **대상** Kali → Metasploitable2(192.168.60.100).
{: .prompt-tip }

```bash
sudo nmap -sU -p 1-100 192.168.60.100        # UDP 스캔
sudo hping3 --udp -a 10.0.0.1 -s 53 -p 53 192.168.60.100   # 출발지 IP 위조
```
