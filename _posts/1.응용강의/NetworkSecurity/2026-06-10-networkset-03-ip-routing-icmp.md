...
```text
192.168.0.30  =  11000000.10101000.00000000.00011110
                 └──────── 32비트 ────────┘
```
...
`192.168.0.30/24` → 앞 24비트가 네트워크부(마스크 `255.255.255.0`).
...
> 우리 실습망 `192.168.0.0/24`가 사설 대역인 건, **인터넷으로 라우팅되지 않아 격리에 안전**하기 때문입니다(01차시 격리 원칙의 기술적 근거).
...
```text
192.168.0.30  AND  255.255.255.0  = 192.168.0.0
192.168.0.10  AND  255.255.255.0  = 192.168.0.0   → 같음 → 같은 LAN(스위치로 직접) ✓
...
```text
default via 192.168.0.1 dev eth0     ← 모르는 목적지는 기본 게이트웨이로
192.168.0.0/24 dev eth0 scope link   ← 이 대역은 직접 연결
```
...
| 내부 | 변환 후(공인) |
|---|---|
| 192.168.0.30:5000 | 203.0.113.5:**40001** |
| 192.168.0.31:5000 | 203.0.113.5:**40002** |
...
traceroute -I 192.168.0.30   # ICMP 기반 강제
```
...
편지의 반송 주소를 거짓으로 쓰는 것과 같습니다. IP 헤더의 출발지 칸은 **보내는 쪽이 자유롭게 적고, 받는 쪽·중간 라우터 누구도 확인하지 않습니다.** 그래서 출발지를 `192.168.0.99`로 위조해도 패킷은 그대로 갑니다 — 응답이 엉뚱한 곳(또는 아무 데도)으로 갈 뿐.
...
sudo hping3 --icmp -a 192.168.0.99 -c 3 192.168.0.30
```

Ubuntu의 `tcpdump`에는 출발지가 `192.168.0.99`로 찍히지만, **그 호스트는 존재하지 않을 수도 있습니다.** 대상은 위조된 주소로 응답을 보내고, 진짜 공격자는 숨습니다.
