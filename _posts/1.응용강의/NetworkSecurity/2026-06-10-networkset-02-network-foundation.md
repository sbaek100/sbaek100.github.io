...
웹 브라우저에서 `http://192.168.0.30`을 입력하면 대상 서버의 웹 페이지가 열립니다. 겉으로는 주소 하나를 입력한 것처럼 보이지만, 실제로는 **서로 다른 세 종류의 주소**(MAC·IP·Port)와 **여러 계층의 규칙**이 동시에 움직입니다.

```mermaid
sequenceDiagram
    participant User as "사용자 브라우저"
    participant Kali as "Kali 192.168.0.10"
    participant Net as "가상 스위치"
    participant Victim as "Ubuntu 192.168.0.30"
    User->>Kali: URL 입력 (http://192.168.0.30)
    Kali->>Net: 목적지 MAC 주소로 프레임 전송
    Net->>Victim: IP 192.168.0.30으로 패킷 전달
...
| IP 주소 | 집 주소 | **32비트**(IPv4)/128비트(IPv6) | `192.168.0.30` | 최종 목적지까지 유지(보통) |
...
> **사전 준비**
> - Kali(`192.168.0.10`)와 Ubuntu(`192.168.0.30`)가 같은 내부망에서 ping이 되는 상태(01차시).
...
```bash
sudo tcpdump -i eth1 -nn -e host 192.168.0.30
```
...
10:24:01.123456 08:00:27:aa:aa:aa > 08:00:27:bb:bb:bb, IPv4,
    192.168.0.10.49152 > 192.168.0.30.80: Flags [S], seq 0 ...
...
| `192.168.0.10` `> 192.168.0.30` | 3계층 | 출발/도착 **IP** |
| `.49152` `> .80` | 4계층 | 출발(임시 포트)/도착 **Port** |
| `Flags [S]` | 4계층 | TCP SYN — 연결 시작(04차시) |

### STEP 4. Wireshark로 계층 펼쳐 보기 (선택)

`sudo wireshark` → `eth1` → 표시 필터 `ip.addr == 192.168.0.30 && tcp.port == 80` → 패킷 클릭 → 상세창에서 `Ethernet II`(MAC) / `Internet Protocol`(IP) / `Transmission Control Protocol`(Port)을 차례로 펼칩니다. **STEP 3의 텍스트가 계층 트리로 보이는 것**을 확인합니다.
...
> **문제 해결**
> - 패킷이 안 잡힘 → 인터페이스명 확인(`ip addr`), `host 192.168.0.30` 오타 확인.
> - MAC이 안 보임 → `-e` 옵션 빠짐.
