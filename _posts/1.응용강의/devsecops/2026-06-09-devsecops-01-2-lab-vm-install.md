...
| VM | 내부 IP | 메모리 권장 |
|---|---|---|
| **build** | 192.168.62.10 | 2048 MB |
| **dev** | 192.168.62.20 | 1024 MB |
| **prod** | 192.168.62.30 | 1024 MB |
...
아래처럼 작성합니다. **`build`는 192.168.62.10**, dev는 `.20`, prod는 `.30` 으로 VM마다 주소만 바꿉니다.
...
`inet 192.168.62.10/24` (build 기준)가 보이면 성공입니다. dev·prod도 같은 방식으로 `.20`, `.30` 확인.
