...
flowchart TB
    subgraph WIN["내 윈도우 PC (호스트)"]
        subgraph VB["VirtualBox (무료)"]
            B["build VM<br/>빌드서버<br/>Gitea + Jenkins<br/>192.168.62.10"]
            D["dev VM<br/>개발계<br/>192.168.62.20"]
            P["prod VM<br/>운영계<br/>192.168.62.30"]
            B -- "자동 배포" --> D
            B -. "승인 후 배포" .-> P
        end
    end
...
| VM | 역할 | IP(내부망) | 비유 |
|---|---|---|---|
| **build** | 빌드서버 | 192.168.62.10 | "자동화 공장" |
| **dev** | 개발계 | 192.168.62.20 | "실험실 매대" |
| **prod** | 운영계 | 192.168.62.30 | "실제 매장 진열대" |

> **왜 `192.168.62` 대역인가 — 다른 과정과의 주소 분리**
> 이 과정은 VM을 **세 대** 동시에 켜 두고 서로 배포를 주고받으므로, 다른 과정의 가상 머신과 주소가 겹치면 배포가 엉뚱한 서버로 갈 수 있습니다. 그래서 전용 대역을 배정했습니다.
>
> | 대역 | 과정 |
> |---|---|
...
> | **`192.168.62.0/24`** | **DevSecOps(이 과정)** |
>
