...
> **📜 역사 한 토막 — 인증기관이 해킹당하면 벌어지는 일: DigiNotar의 몰락(2011)**
>
> 네덜란드의 인증기관(CA) DigiNotar는 2011년 여름 해킹을 당했고, 공격자는 `*.google.com`을 포함한 **500여 장의 가짜 인증서**를 마음대로 발급해 갔다. 이 가짜 인증서는 이란에서 약 30만 명의 Gmail 통신을 중간자(MITM) 방식으로 감청하는 데 악용된 정황이 드러났다.
>
> 각 브라우저 제조사는 DigiNotar를 신뢰 목록에서 즉시 삭제했고, 정부 전자서비스용 인증서까지 발급하던 이 회사는 **몇 주 만에 파산**했다. 네덜란드 정부는 한동안 자국 전자행정 시스템의 신뢰를 복구하느라 비상이 걸렸다.
>
> 이 사건이 남긴 교훈은 이것이다. **PKI의 약점은 수학이 아니라 신뢰 그 자체다.** 신뢰 체인의 뿌리인 CA 하나가 뚫리면 그 아래 전부가 위험해진다. 이후 모든 인증서 발급 내역을 공개 장부에 기록해 누구나 감시할 수 있게 하는 **Certificate Transparency** 제도가 도입되는 계기가 되었다.
>
> *근거: 네덜란드 정부 의뢰 Fox-IT 조사 보고서 "Operation Black Tulip"(2011) — 위조 인증서 531장, 이란 내 약 30만 IP의 접속 확인.*
{: .prompt-tip }
