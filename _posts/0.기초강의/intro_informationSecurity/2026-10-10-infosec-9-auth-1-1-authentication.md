...
> **📜 역사 한 토막 — 최초의 패스워드, 그리고 최초의 패스워드 도난(1962)**
>
> 컴퓨터 패스워드는 1960년대 초 MIT의 시분할 시스템 **CTSS**에서 태어났다. 여러 사람이 한 컴퓨터를 나눠 쓰게 되자 "내 파일과 내 사용 시간"을 구분할 장치가 필요했던 것이다. 그런데 이 최초의 패스워드 시스템은 **최초의 패스워드 도난 기록도 함께** 남겼다. 주당 4시간의 사용 시간이 부족했던 박사과정생 앨런 셰어가, 관리 문서를 출력하는 정상 기능을 이용해 **패스워드 파일 전체를 인쇄**해 버린 것이다. 그는 40여 년이 지난 뒤에야 이 일을 웃으며 자백했다.
>
> 그로부터 반세기 뒤인 2009년, 게임 소셜 서비스 **RockYou**가 해킹당해 **3,200만 개의 비밀번호가 평문 그대로** 유출되었다(해시조차 하지 않았다). 위 5.4의 "123456" 통계가 바로 이런 유출 데이터 분석에서 나온 것이고, 이때 만들어진 단어 목록(rockyou.txt)은 오늘날까지 **사전 공격의 표준 단어장**으로 쓰인다. 60년 전의 교훈("패스워드는 훔쳐진다")이 규모만 커진 채 반복되는 셈이다.
>
> *근거: F. Corbató·A. Scherr의 회고 인터뷰(Wired, 2012 등); Imperva 분석 보고서 "Consumer Password Worst Practices"(2010) — RockYou 유출 데이터 분석.*
{: .prompt-tip }
