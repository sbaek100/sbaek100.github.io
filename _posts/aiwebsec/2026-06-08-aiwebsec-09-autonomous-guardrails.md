...
> **🎯 우리가 지금 왜 이걸 하나요?**  
> 도구가 강해질수록 **실수도 커져요.** 자동 점검기는 사람보다 훨씬 빠르고 많이 움직이기 때문에, 한 번 잘못 겨누면 피해도 순식간이에요. 이번 강의에서는 지금까지 흩어져 있던 도구를 하나로 묶어 **스스로 점검하게** 만들면서, 동시에 **사고를 막는 안전장치(가드레일)**를 함께 답니다. "강력함"과 "안전함"은 반드시 같이 가야 한다는 걸 코드로 배우는 시간이에요.
{: .prompt-info }
