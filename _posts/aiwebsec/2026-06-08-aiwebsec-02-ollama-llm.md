...
> **🎯 우리가 지금 왜 이걸 하나요?**  
> AI 에이전트의 **"두뇌"**를 우리 컴퓨터 안에 들여놓는 단계예요. 외부 AI 서비스에 매번 요금을 내거나 점검 데이터를 보내지 않고, **무료로 · 내 PC 안에서 · 인터넷 없이도** 돌아가는 LLM을 갖추는 거죠. 지금은 똑똑한 두뇌가 아니라 **작고 빠른 두뇌**면 충분해요. 우리는 "성능"이 아니라 "구조가 돌아가는지"부터 확인할 거니까요. 좋은 모델은 나중에 한 줄만 바꾸면 끼울 수 있어요.
{: .prompt-info }
