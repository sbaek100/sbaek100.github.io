...
> **🎯 우리가 지금 왜 이걸 하나요?**  
> 자동화란 결국 **"사람이 손으로 하던 일을 프로그램이 대신하는 것"**이에요. 지금까지는 우리가 터미널에 직접 질문을 쳤지만, 이제부터는 **우리가 만든 코드가 LLM에게 말을 걸게** 만들 거예요. 이 작은 연결 고리가 생겨야, 나중에 AI가 사람 없이도 스스로 점검을 이어 갈 수 있어요. 오늘이 그 첫 코드예요.
{: .prompt-info }
