...
> **OWASP Top 10 2021에서는 A01 Broken Access Control(접근 통제 실패)에 통합**되었다.  
> "인증된 사용자라도, 그 요청이 정말 본인 의도인지"를 검증하지 못한 접근 통제 결함으로 본다.
{: .prompt-info }
...
> 핵심: Referer 검증(Medium)은 우회 가능, 토큰(High)은 XSS에 취약.  
> Impossible은 **토큰 + 민감 작업 시 재인증**을 결합해 방어한다. CSRF 방어는 항상 **XSS 방어와 함께** 가야 한다.
{: .prompt-tip }
...
> High의 한계: 토큰이 같은 페이지(같은 출처)에 노출되므로, **XSS 한 곳만 있어도** 토큰을 읽어 우회된다.  
> 따라서 CSRF 방어는 XSS 방어(02장)와 반드시 병행해야 한다.
{: .prompt-warning }
...
Impossible 레벨은 Anti-CSRF 토큰에 더해 **현재 비밀번호 재확인**을 요구한다.
...
CSRF 공격자는 피해자의 **현재 비밀번호를 알 수 없으므로**, 토큰을 어찌어찌 얻더라도 요청이 거부된다.  
민감한 상태 변경(비밀번호 변경, 송금 등)에 **재인증을 결합**하는 것이 가장 견고한 패턴이다.
