...
```mermaid
graph LR
    A["CHAR(10)에 'AB' 저장"] --> B["AB________\n(10칸 고정 할당)"]
    C["VARCHAR(10)에 'AB' 저장"] --> D["AB\n(2칸만 할당)"]
```
...
flowchart TD
    A[관계 설계 시작] --> B{관계 종류?}
    B -->|1:1| C["외래키 + UNIQUE 제약 조건\n사용 빈도 낮은 쪽에 배치"]
    B -->|1:N| D["외래키를 N 쪽 테이블에 배치\nUNIQUE 없음"]
    B -->|N:M| E["중간 테이블 생성\n양쪽 PK를 FK로 참조"]
    C --> F[참조 무결성 자동 보장]
    D --> F
    E --> F
