---
title: (5일차) 네트워크 기초 13) - 전체 흐름 복습 — PC에서 서버까지
...
### 이 글을 읽을 때 추천하는 복습 방법

아래처럼 Packet Tracer 화면을 같이 띄워 두고 보면 훨씬 잘 들어옴.

1. Realtime Mode에서 먼저 웹 접속이 되는지 확인
2. `Simulation`으로 바꿈
3. `ARP`, `TCP`, `HTTP`만 필터로 남김
4. `PC0` 브라우저에서 `http://192.168.2.100` 입력
5. `Capture/Forward`를 누르며 지금 글의 ①~⑤ 설명과 비교

> 이 글은 이론 복습 글이지만, Packet Tracer 화면과 같이 보면 가장 이해가 잘 됨.
...
- Event List에서 `ARP`를 찾기
- 시작 장비가 `PC0`인지 보기
- 같은 LAN 안의 Server0 MAC이 아니라, 먼저 게이트웨이 쪽으로 보내는 흐름 이해하기
- Reply가 돌아온 뒤에 TCP가 시작되는지 보기
...
- `TCP`가 ARP 뒤에 나오는지
- 방향이 `PC0 → Router0` 쪽에서 시작되는지
- `OSI Model`에서 Layer 4가 TCP인지
- 가능하면 목적지 포트가 80인지
...
- IP 목적지는 그대로인지 보기
- TTL이 줄어드는지 보기
- 라우터를 지나면서 다음 단계로 넘기는 장비라는 감각 잡기
...
- `Router1`에서도 같은 방식으로 확인
- `show ip route`를 같이 열어 두고 `192.168.2.0`이 직접 연결인지 연결해서 보기
- 이제 목적지가 같은 LAN 안이므로 Server0 쪽으로 전달된다는 점 이해하기
...
- `HTTP GET`이 Server0에 도착하는지
- `HTTP 200 OK` 응답이 다시 돌아가기 시작하는지
- 응답도 요청과 비슷한 길을 반대로 간다는 점 보기
...
### 14강 전에 학생이 미리 체크하면 좋은 것

- 라우터 시리얼 포트가 보이는지
- `clock rate`를 어느 쪽에 넣는지 기억하는지
- `show ip interface brief`, `show ip route` 두 명령을 어디서 치는지 기억하는지
- `Web Browser`, `Command Prompt`, `Simulation`, `Edit Filters` 버튼 위치를 알고 있는지
