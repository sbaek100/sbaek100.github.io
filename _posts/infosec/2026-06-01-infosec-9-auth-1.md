...
![|697x338](/assets/img/posts/2026-06-01-infosec-9-auth-1-1780901992081.png)
...
### 7.4 OTP(One-Time Password): 한 번 쓰고 버리는 비밀번호
...
#### (1) 시간 동기화 방식 (Time-synchronous, TOTP)
...
#### (2) 이벤트(카운터) 동기화 방식 (Event-synchronous, HOTP)
...
### 8.1 Kerberos: 티켓 기반 네트워크 인증 프로토콜

**Kerberos(케르베로스)** 는 MIT에서 개발한 **네트워크 인증 프로토콜**로, SSO를 구현하는 대표적인 기술이다.  
이름은 그리스 신화에 나오는 머리 셋 달린 지옥문지기 개에서 따왔다 — 그만큼 **"믿을 수 있는 문지기"** 역할을 한다.

- **비밀키(대칭키) 암호 기반의 온라인 인증·키 분배** 방식이다.
- 신뢰할 수 있는 **제3자(KDC)** 를 두고, **"티켓(Ticket)"** 이라는 증표로 인증한다.
- **비밀번호를 네트워크로 직접 전송하지 않는다** → 도청·재전송 공격에 강하다.
- 실제 활용: **윈도우 액티브 디렉터리(Active Directory)** 도메인 로그인 등.

#### 구성 요소

| 구성 요소 | 역할 |
|---|---|
| **KDC** (Key Distribution Center) | 인증의 중심. 아래 AS와 TGS로 구성된 신뢰된 제3자 |
| └ **AS** (Authentication Server) | 사용자 신원을 확인하고 **TGT**(티켓을 받기 위한 티켓)를 발급 |
| └ **TGS** (Ticket Granting Server) | TGT를 검증하고 실제 **서비스 이용 티켓**을 발급 |
| **Client** | 인증을 요청하는 사용자 |
| **Service Server** | 사용자가 이용하려는 실제 서비스 |

#### 인증 흐름 (간단히)

```mermaid
flowchart LR
C["Client"] -->|"① 로그인 요청"| AS["AS<br/>(인증 서버)"]
AS -->|"② TGT 발급"| C
C -->|"③ TGT 제시 + 서비스 요청"| TGS["TGS<br/>(티켓 발급 서버)"]
TGS -->|"④ 서비스 티켓 발급"| C
C -->|"⑤ 서비스 티켓 제시"| S["Service<br/>Server"]
```

1. 사용자가 **AS**에 로그인 → AS가 **TGT**(Ticket Granting Ticket)를 발급한다.
2. 클라이언트가 TGT를 들고 **TGS**에 특정 서비스를 요청 → TGS가 **서비스 티켓**을 발급한다.
3. 클라이언트가 서비스 티켓을 **Service Server**에 제시하고 서비스를 이용한다.

→ 한 번 TGT를 받으면 이후 서비스마다 비밀번호를 다시 넣지 않아도 되므로 **SSO가 실현**된다.

#### 특징과 한계

| 구분 | 내용 |
|---|---|
| **보장하는 것** | 대칭키 암호 기반으로 데이터의 **기밀성·무결성**을 보장 |
| **목적** | **인증된 클라이언트만** 서버에 접근하도록 하는 것 |
| **단일 실패 지점(SPOF)** | **KDC에 장애가 나면 전체 인증 서비스가 마비**된다 |
| **시간 동기화 민감** | 티켓에 유효시간이 있어, 서버 간 **시계가 어긋나면 인증 실패** |
